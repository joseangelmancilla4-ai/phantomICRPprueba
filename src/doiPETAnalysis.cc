...
// Nueva:
void doiPETAnalysis::ReadOut(G4int blkID, G4int cryID, G4double interTime, G4double timeAnnih, G4ThreeVector interPosLocal, G4double edep, G4double truthEnergy, G4ThreeVector globalPos)
//void doiPETAnalysis::ReadOut(G4int blkID, G4int cryID, G4double interTime, G4double timeAnnih, G4ThreeVector interPos, G4double edep)
{
	...
	if(totalEdep>lowerThreshold && totalEdep<upperThreshold ){
		...
		if(getCoincidenceData){
			...
			// ***** AÑADE ESTO AQUÍ *****
                        truthEnergy_coin.push_back(truthEnergy); // Guardar energía "truth"
                        xPos_coin.push_back(globalPos.x()); // Guardar posición X global
                        yPos_coin.push_back(globalPos.y());   // Guardar posición Y global
                        zPos_coin.push_back(globalPos.z()); // Guardar posición Z global
	...
	if(getCoincidenceData){
		...
		for(G4int i=0; i<2; i++){
			...
			if(i==0){
				...
				truthEnergy0 = truthEnergy_coin[0];
                                xPos0 = xPos_coin[0];
                                yPos0 = yPos_coin[0];
                                zPos0 = zPos_coin[0];
			...
			if(i==1){
				...
				eventID1				= eventID_coin[1];
                                truthEnergy1 = truthEnergy_coin[1];
                                xPos1 = xPos_coin[1];
                                yPos1 = yPos_coin[1];
                                zPos1 = zPos_coin[1];
		...
		ofs << eventID0 << " "                  // EventID1
        << spositionX << " " << spositionY << " " << spositionZ << " " // Origen (Aniquilación)
        << truthEnergy0 / keV << " "        // Energía "Truth" 1 (en keV)
        << xPos0 / mm << " " << yPos0 / mm << " " << zPos0 / mm << " " // Posición Global Hit 1 (en mm)
        << totalEdep0 / keV << " "          // Energía Detectada 1 (en keV)
        << crystalID_axial0 << " "          // Índice Axial 1
        << crystalID_tangential0 << " "     // Índice Tangencial 1
        << blockID0 << " "                  // Índice Bloque 1
        << DOI_ID0 << " "                   // Índice DOI 1
        << timeStamp0 / ns << " "           // Timestamp 1 (en ns)
        // --- Repetir para el Hit 2 ---
        << eventID1 << " "                  // EventID2
        << spositionX << " " << spositionY << " " << spositionZ << " " // Origen (Aniquilación)
        << truthEnergy1 / keV << " "        // Energía "Truth" 2 (en keV)
        << xPos1 / mm << " " << yPos1 / mm << " " << zPos1 / mm << " " // Posición Global Hit 2 (en mm)
        << totalEdep1 / keV << " "          // Energía Detectada 2 (en keV)
        << crystalID_axial1 << " "          // Índice Axial 2
        << crystalID_tangential1 << " "     // Índice Tangencial 2
        << blockID1 << " "                  // Índice Bloque 2
        << DOI_ID1 << " "                   // Índice DOI 2
        << timeStamp1 / ns << " "           // Timestamp 2 (en ns)
        << G4endl;
// ********************************************
